...
@section('content')
  ...
  <div class="col-lg-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <div class="card-title pl-1"><strong>Güncel Eğitimlerim</strong> </div>
                  <div class="col-12">
                        ...
                        <tbody>
                          @if(!empty($list))
                            @foreach ($list as $key => $value)
                              @if (!empty($value->education) && $value->education->count()>0 )
                                <tr>
                                  ...
                                  <td>
                                    ...
                                    @endphp
                                    @if (!empty($inf->isSuccess))
                                        @if ($inf->isSuccess=="no")
                                          @if ($inf->educationTime=="00:00:00")
                                            Eğitim Atandı
                                          @else
                                            Devam Ediyor (Devam Süresi {{$inf->educationTime}})
                                          @endif
                                  ...
                                  <!--   $totalDuration = $finishTime->diffInSeconds($startTime); -->
                                      <a href='/education/watch/{{$value->education->id}}' class="text-danger"><i class="fas fa-chalkboard-teacher"></i></a>
                                  </td>
                                </tr>
                              @endif
                            @endforeach
      ...
      <div class="modal-dialog  modal-xxl" role="document">
        <div class="modal-content">
          <div class="modal-header bg-qpm-dark text-white">
            <span class="modal-title ml-1 pl-1">Eğitim </span>
