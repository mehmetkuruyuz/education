...
<div class="container-fluid my-3">
  <div class="row justify-content-center">
    <div class="col-md-12">
      <div class="card">
        <div class="card-body">
          <div class="card-title pl-1"><strong>Sipariş Edilecek Talepler</strong>

          </div>
          <form action="/teklif/index" method="post" >
            {{csrf_field()}}
          ...
          <form action="/teklif/onayla" method="post" name="x">
                                  ...
                                  <td colspan="10">
                                        <table class="table">
                                          <thead>
                                            ...
                                            @foreach ($vlo->teklifurun as $top => $deger)
                                              <tr>
                                                <td>
                                                ...
                                                <td>
                                                  ...
                                                  @elseif ($deger->status=="siparisedildi")
                                                  Sipariş Edildi
                                                  @else
                                                    Tedarik Edilmiş.
                                                @endif
            ...
            <div class="col-12">
              <button class="btn btn-danger d-none" id="opensave" type="button"  data-toggle="modal" data-target="#siparisedilme" >Sipariş Edildi</button>
            </div>


            <div class="modal" tabindex="-1" role="dialog" id="siparisedilme">
              <div class="modal-dialog  modal-lg" role="document">
                <div class="modal-content pb-5">
                  <div class="modal-header bg-qpm-dark text-white">
                    <span class="modal-title ml-1 pl-1">Sipariş Bilgileri</span>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close" class="text-white">
                      <span aria-hidden="true">&times;</span>
                    </button>
                  </div>
                  <div class="modal-body " id="siparisedilmebody">
                    <div class="form-group my-0">
                        <label class="smallx ml-3">Sipariş Kodu</label>
                        <div class="col-md-12">
                              <input id="" type="text" class="form-control" name="satinalmakodu" id="satinalmakodu" required />
                        </div>
                    </div>
                    <div class="form-group my-0">
                        <label class="smallx ml-3">Tedarikçi</label>
                        <div class="col-md-12">
                              <select name="onerilentedarikci" class="selectmulti form-control" style="width:100%" id="siparisfirma">
                                  @if (!empty($firmalist))
                                      @foreach ($firmalist as $key => $value)
                                          <option value="{{$value->firmakodu}} - {{$value->firmaadi}}">{{$value->firmakodu}} - {{$value->firmaadi}}</option>
                                      @endforeach
                                  @endif
                              </select>
                        </div>
                  ...
                  <div class="modal-footer text-left">
                      <button class="btn btn-danger" type="submit" onclick="send">Kaydet</button>
                  </div>
                </div>
              </div>
            </div>
